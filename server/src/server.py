# -*- coding: utf-8 -*-
"""
Agora Agent & Token Service — STT Vendors Recipe

HTTP APIs:
- GET  /get_config     -> Generate connection config
- POST /startAgent     -> Start the agent (with the selected STT vendor)
...
# FastAPI application
app = FastAPI(
    title="Agora STT Vendors Recipe Service",
    version="1.0.0",
    description="Agora Conversational AI — STT vendor switchboard",
)
...
def _generate_channel_name() -> str:
    return f"sttvendors-{int(time.time())}-{random.randint(1000, 9999)}"
...
async def list_vendors():
    """List the selectable STT vendors for the in-UI switcher."""
    import vendors as registry
    default = os.getenv("STT_VENDOR", "deepgram")
    return {
        "code": 0,
        "data": {
            "default": default,
